...
describe("token exports (SPEC §5–§8 smoke)", () => {
  ...
  it("exports cascade physics from spec §8.6", () => {
    expect(CASCADE.gravity).toBe(2200);
    expect(CASCADE.vxMin).toBe(-420);
    expect(CASCADE.vxMax).toBe(420);
    expect(CASCADE.vyMin).toBe(-360);
    expect(CASCADE.vyMax).toBe(-120);
    expect(CASCADE.floorRestitution).toBe(0.82);
    expect(CASCADE.wallRestitution).toBe(0.7);
  });
